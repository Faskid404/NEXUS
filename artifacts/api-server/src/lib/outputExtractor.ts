...
/* ── Core extraction ────────────────────────────────────────── */
/* ── WAF / CDN block-page guard ─────────────────────────────────────────────
   Cloudflare Ray ID pages, Imperva incident pages, DataDome JS challenges,
   PerimeterX blocks, Kasada, AWS WAF denials — none can contain real command
   output, so we must hard-skip them BEFORE any pattern matching to prevent
   false positives from WAF error copy that happens to mention "uid" or "root".
   ─────────────────────────────────────────────────────────────────────────── */
const WAF_BLOCK_SIGNATURES: string[] = [
  "cloudflare ray id:",
  "cf-ray:",
  "error 1020",
  "error 1015",
  ...
  "your access to this page has been denied",
  "this request has been blocked",
  "the request has been blocked",
  "request was rejected by the web application firewall",
...
function isWafBlockPage(body: string): boolean {
  const b = body.toLowerCase();
  const trimmed = body.trimStart();
  // Only flag actual HTML pages — JSON/text RCE output is never a WAF block
  if (!/^<!DOCTYPE\s+html/i.test(trimmed) && !/^<html[\s>]/i.test(trimmed)) return false;
  return WAF_BLOCK_SIGNATURES.some(sig => b.includes(sig));
}
...
export function extractCommandOutput(
  ...
  /* ── Strip HTML once at function entry ───────────────────────────────────
     MUST be before step 0 — previously declared in step 1, causing a
     ReferenceError that crashed every call to extractCommandOutput.       */
  ...
  // `plain` already declared above — hoisted to function entry
  ...
  /* ── WAF block page guard (runs before HTML guard) ──────────
     Hard-skip Cloudflare, Imperva, DataDome, PerimeterX, Kasada
     block pages — they can never contain real RCE output, and
     their error copy (mentioning "uid", "blocked", etc.) must
     not match RCE patterns under any circumstances.           */
  if (isWafBlockPage(body)) return null;
...
export function detectHoneypotHint(
  ...
  // Known honeypot framework signatures
  ...
  // Error pages that look like successful injections (Cowrie, Dionaea, etc.)
  ...
  // Modern bot-detection / WAF sandboxes that return fake successful output
  // to fingerprint the attacker tool before blocking the real session
  if (/__ddg|_pxhd|kasada|px\.js|shape_c=|arkose/i.test(body) &&
      /uid=|root:|passwd/i.test(body)) {
    return { likelyHoneypot: true, reason: "Bot-detection challenge page with fake RCE output — DataDome/PerimeterX/Kasada deception" };
  }
  // Responses that arrive < 5 ms with perfect uid=0 output — physically impossible
  if (status === 200 && /uid=0\(root\)/.test(body) && body.length < 80 && body.trim().split("\n").length <= 3) {
    return { likelyHoneypot: true, reason: "Impossibly fast + clean root output — likely a CDN-cached honeypot response" };
  }
