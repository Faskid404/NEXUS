...
export const MutationScannerRequestSchema = z.object({
  ...
  mutationRate: z.coerce.number().min(0).max(1).optional(),
  crossoverRate:z.coerce.number().min(0).max(1).optional(),
  timeout:      z.coerce.number().int().min(500).max(30_000).optional(),
