...
export function ansiLine(text: string): string {
  ...
  if (/SUCCESS|success|EXEC/.test(text))   return `\x1b[32m${text}\x1b[0m`;
  if (/BLOCKED|blocked|WAF|waf/.test(text))return `\x1b[31m${text}\x1b[0m`;
  if (/SKIP|skip|FILTERED/.test(text))     return `\x1b[90m${text}\x1b[0m`;
  if (/TIMING|timing|SLEEP/.test(text))    return `\x1b[32m${text}\x1b[0m`;
...
const NexusTerminal = forwardRef<NexusTerminalHandle, Props>(function NexusTerminal(
  ...
  useEffect(() => {
    ...
    term.writeln("\x1b[1;31m NEXUSFORGE \x1b[0m\x1b[90m─── Auto-Escalation Engine Ready\x1b[0m");
    term.writeln("\x1b[90m─────────────────────────────────────────────────────────\x1b[0m");
    term.writeln("\x1b[90m Enter target URL and configure parameters, then LAUNCH.\x1b[0m");
    term.writeln("");
